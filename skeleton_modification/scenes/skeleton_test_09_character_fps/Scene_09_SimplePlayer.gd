...
var bullet_scene:PackedScene = preload("res://Scenes/Skeleton_Test_09/Test_09_Bullet.tscn");
var gun_end:Node3D;
...
func _process(delta:float):
	...
	if Input.is_action_pressed("fire"):
		if (fire_timer <= 0):
			...
			gun_shake_strength = 1;
			gun_shake_rotation.x = deg2rad(rand_range(-60, 60));
			gun_shake_rotation.y = deg2rad(rand_range(-60, 60));
...
func _input(event):
	if event is InputEventMouseMotion and Input.get_mouse_mode() == Input.MOUSE_MODE_CAPTURED:
		...
		var camera_rot = head_node.rotation_degrees
		camera_rot.x = clamp(camera_rot.x, -70, 70)
		head_node.rotation_degrees = camera_rot
